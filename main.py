...
CITY_TO_AIRPORT_CODE = {
    ...
    "Ahmedabad": "AMD", "Kochi": "COK", "Jaipur": "JAI", "Chandigarh": "IXC",
    "Guwahati": "GAU", "New York": "JFK", "Los Angeles": "LAX", "San Francisco": "SFO",
    "London": "LHR", "Paris": "CDG", "Berlin": "BER", "Tokyo": "HND", "Beijing": "PEK",
    "Shanghai": "PVG", "Dubai": "DXB", "Sydney": "SYD", "Singapore": "SIN", "Toronto": "YYZ",
    "Mexico City": "MEX", "Cairo": "CAI", "Moscow": "SVO", "Istanbul": "IST", "Seoul": "ICN",
    "Bangkok": "BKK", "Cape Town": "CPT", "Rio de Janeiro": "GIG", "Buenos Aires": "EZE"
}
...
tools = [
    Tool(name="FlightSearch", func=FlightSearchTool()._run, description="Fetch flight details based on origin, destination, and date."),
    Tool(name="WeatherReport", func=WeatherReport()._run, description="Fetches the weather report of the destination."),
    Tool(name="SearxSearch", func=search.run, description="Search the tourism destination with tourist places."),
    Tool(name="WikipediaSummary", func=wikipedia_tool.run, description="Fetches the Wikipedia summary for a given destination.")
]
...
def attempts() -> int:
    ...
    if not os.path.exists('attempts.json'):
        ...
        with open("attempts.json", 'w') as file:
            json.dump({'attempts': 5, 'last_updated': today}, file, indent=2)

    # If not empty, load the JSON data
    with open('attempts.json', 'r') as file:
        data = json.load(file)

    updated_attempts = data.get("last_updated")
    if today != updated_attempts:
        # Only reset attempts to 5 if it's a new day
        data['attempts'] = 5
        data['last_updated'] = today
        update_attempts(data['attempts'])
...
def update_attempts(no_items: int):
    with open('attempts.json', 'w') as file:
        json.dump({'attempts': no_items, 'last_updated': datetime.today().strftime("%y-%m-%d")}, file)
...
user_input = st.sidebar.text_input("Enter your email ID: ", placeholder="EmailID")
...
validate_email = check_email(user_input)
# When submit button is pressed, execute the logic
if submit_button and origin_input and destination_input and date_input and validate_email:
    if no_attempts > 0:
